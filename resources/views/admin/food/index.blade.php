...
<head>
    ...
    <title>Manage Food - Admin Dashboard</title>
            ...
            <div class="max-w-7xl mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100/50">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-1">Total Food</p>
                                <p class="text-3xl font-semibold bg-gradient-to-r from-blue-600 to-blue-400 bg-clip-text text-transparent">{{ $totalFood }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100/50">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-1">Available Food</p>
                                <p class="text-3xl font-semibold bg-gradient-to-r from-green-600 to-green-400 bg-clip-text text-transparent">{{ $availableFood }}</p>
                            </div>
                    ...
                    <div class="bg-gradient-to-br from-white to-gray-50 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 border border-gray-100/50">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-500 mb-1">Sold Out Food</p>
                                <p class="text-3xl font-semibold bg-gradient-to-r from-red-600 to-red-400 bg-clip-text text-transparent">{{ $totalFood - $availableFood }}</p>
                            </div>
                ...
                <div class="bg-white rounded-lg p-6 shadow-sm border border-gray-200">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-xl font-semibold text-gray-900">Manage Food</h2>
                    ...
                    <!-- Filters -->
                    <div class="mb-6">
                        <form action="{{ route('admin.food.index') }}" method="GET" class="flex gap-4">
                            <select name="pet_type" class="rounded-md border border-gray-300 px-6 py-2 text-sm text-gray-900">
                                <option value="">Pet Type</option>
                                <option value="dog" {{ request('pet_type') == 'dog' ? 'selected' : '' }}>Dog</option>
                                <option value="cat" {{ request('pet_type') == 'cat' ? 'selected' : '' }}>Cat</option>
                            </select>

                            <select name="brand" class="rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-900">
                                <option value="">Brand</option>
                                @foreach($foods->pluck('brand')->unique() as $brand)
                            ...
                            @if(request()->anyFilled(['pet_type', 'brand', 'status']))
                                <a href="{{ route('admin.food.index') }}" class="px-4 py-2 rounded-md text-sm font-medium text-gray-900 bg-gray-100 hover:bg-gray-200">
                                    Clear Filters
                                </a>
                            @endif
                        </form>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="bg-white overflow-hidden shadow-lg rounded-lg hover:shadow-xl transition-shadow duration-300">
                            ...
                            <div class="relative h-48">
                                <img src="{{ asset('storage/' . $food->image1) }}" alt="{{ $food->name }}" class="w-full h-full object-cover">
                                <div class="absolute top-2 right-2">
                                    <span class="px-2 py-1 text-xs font-semibold rounded-full 
                                        @if($food->stock > 0) bg-green-100 text-green-800
                                        @else bg-red-100 text-red-800 @endif">
                                        @if($food->stock > 0) In-Stock @else Out of Stock @endif
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="p-4 border-t">
                                <div class="flex justify-between items-start">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">{{ $food->brand }} • {{ $food->pet_type }}</p>
                                    </div>
                                    <span class="text-lg font-semibold text-blue-800">₱{{ number_format($food->price, 2) }}</span>
                                </div>

                                <div class="mt-4 grid grid-cols-2 gap-4 text-sm text-gray-600">
                                    <div>
                                        <span class="font-medium">Stock:</span> {{ $food->stock }}
                                    </div>
                                    <div>
                                        <span class="font-medium">Weight:</span> {{ $food->weight }}
                                    </div>
                                    <div>
                                        <span class="font-medium">Flavor:</span> {{ $food->flavor }}
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 flex justify-between items-center">
                                    ...
                                    <form action="{{ route('admin.food.destroy', $food->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
