...
<head>
    ...
    <title>Admin | Edit Accessory</title>
...
<div class="sidebar expanded fixed left-4 top-4 h-[calc(100vh-2rem)] bg-white rounded-2xl p-4 z-50">
    ...
    <nav class="space-y-2 flex flex-col h-full">
        ...
        <div class="main-content flex-1 ml-[270px] p-8">
            <div class="max-w-7xl mx-auto  border-l border-gray-200">
                <div class="bg-white shadow-sm rounded-lg p-6">
                    <div class="flex justify-between items-center mb-6">
                        <h2 class="text-2xl font-semibold text-gray-800">Edit Accessory</h2>
                        <a href="{{ route('admin.accessories.index') }}" class="text-blue-600 hover:text-blue-800">
                            Back to Accessories
                        </a>
                    </div>

                    <form action="{{ route('admin.accessories.update', $accessories->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <!-- Accessory Images -->
                        <div class="space-y-4">
                            <h3 class="text-lg font-medium text-gray-900">Accessory Images</h3>
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                @for ($i = 1; $i <= 5; $i++)
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Image {{ $i }}</label>
                                        <div class="image-preview {{ $accessories->main_image == $i ? 'selected' : '' }}">
                                            @if ($accessories->{'image' . $i})
                                                <img src="{{ asset('storage/' . $accessories->{'image' . $i}) }}" alt="Image {{ $i }}" class="w-full h-48 object-cover rounded-lg">
                                                @if ($accessories->main_image == $i)
                                                    <span class="main-image-badge">Main Image</span>
                                                @endif
                                            @else
                                                <p class="text-gray-500">No image uploaded.</p>
                                            @endif
                                            <input type="file" name="image{{ $i }}" class="hidden" accept="image/*">
                                        </div>
                                        <div class="mt-2">
                                            <button type="button" class="text-sm text-blue-600 hover:text-blue-800" onclick="document.querySelector('input[name=image{{ $i }}]').click()">
                                                Change Image
                                            </button>
                                        </div>
                                        @error('image' . $i)
                        ...
                        <!-- Main Image Selector -->
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Select Main Image (1–5)</label>
                            <select name="main_image" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                @for ($i = 1; $i <= 5; $i++)
                                    <option value="{{ $i }}" {{ $accessories->main_image == $i ? 'selected' : '' }}>Image {{ $i }}</option>
                                @endfor
                            </select>
                            @error('main_image')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="name" value="{{ old('name', $accessories->name) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('name')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="category" value="{{ old('category', $accessories->category) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('category')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" step="0.01" name="price" value="{{ old('price', $accessories->price) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('price')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="stock" value="{{ old('stock', $accessories->stock) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('stock')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="color" value="{{ old('color', $accessories->color) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('color')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="size" value="{{ old('size', $accessories->size) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    @error('size')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
